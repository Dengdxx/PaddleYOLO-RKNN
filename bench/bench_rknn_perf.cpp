 ...
 *   - 后处理时间        若 --postproc 非 none，则使用 NEON 加速候选筛选、dequant 与 mask
 *   - 离线 FPS          多 RKNN context 并发循环，不接摄像头/ROS/真实预处理
 *
 * 后处理路线 (NEON multi-label 阈值扫描 + candidate-first decode):
 *   predist (reg_max=1):
 *     scores: [1,nc,A] i8  → INT8 multi-label 阈值扫描
...
static constexpr int kAutoRoiTiledInt8MaxArea = 8000;
...
static inline float32x4_t neon_exp_fast4(float32x4_t x) {
    const float32x4_t hi = vdupq_n_f32(88.0f);
    const float32x4_t lo = vdupq_n_f32(-88.0f);
    ...
    float32x4_t polynomial = vmlaq_n_f32(vdupq_n_f32(0.0096181291f), fraction,
                                         0.0013333558f);
...
static void neon_sigmoid_inplace(float *data, size_t n) {
    const float32x4_t one = vdupq_n_f32(1.0f);
    size_t i = 0;
    for (; i + 4 <= n; i += 4) {
        const float32x4_t value = vld1q_f32(data + i);
        const float32x4_t exponential = neon_exp_fast4(vnegq_f32(value));
        vst1q_f32(data + i, vdivq_f32(one, vaddq_f32(one, exponential)));
    }
    for (; i < n; ++i) {
        const float value = std::clamp(data[i], -88.0f, 88.0f);
        data[i] = 1.0f / (1.0f + std::exp(-value));
    }
}
...
static void free_postproc_buffers(void) {
    ...
    g_lcandidates = 0;
    g_lclass_seeds = 0;
    g_last_kept = 0; g_last_candidates = 0; g_last_sum_scanned = 0;
    g_last_score_sum_applied = 0;
    g_last_class_anchors = 0; g_last_class_values = 0; g_last_mask_pixels = 0;
    g_last_nms_pairs = 0;
    g_last_mask_active = 0; g_last_proto_roi_area = 0; g_last_mask_hash = 0;
...
static candidate_t *ensure_candidates(int need) {
    if (need > g_lcandidates) {
        free(g_candidates);
        size_t count = (size_t)((need + 7) & ~7);
        g_candidates = (candidate_t *)aligned_alloc(64, count * sizeof(candidate_t));
        g_lcandidates = need;
    }
    return g_candidates;
}
...
/**
 * @brief 确保 multi-label 分类候选 scratch 容量充足。
 * @param need 需要的候选数。
 * @return 当前线程的候选缓冲区。
 */
static paddleyolo_rknn::postprocess::ClassSeed *ensure_class_seeds(int need) {
    if (need > g_lclass_seeds) {
        free(g_class_seeds);
        const size_t count = static_cast<size_t>((need + 7) & ~7);
        g_class_seeds = static_cast<paddleyolo_rknn::postprocess::ClassSeed *>(
            aligned_alloc(64, count * sizeof(*g_class_seeds)));
        g_lclass_seeds = need;
    }
...
static float decode_dfl_side(const int8_t *boxes,
    ...
    for (int k = 0; k < reg_max; k++) {
        size_t index = transposed
                           ? (size_t)anchor * (4 * reg_max) + side * reg_max + k
                           : (size_t)(side * reg_max + k) * anchors + anchor;
        float value = dequant_i8_value(boxes[index], attr->zp, attr->scale);
    ...
    for (int k = 0; k < reg_max; k++) {
        size_t index = transposed
                           ...
                           : (size_t)(side * reg_max + k) * anchors + anchor;
        float value = dequant_i8_value(boxes[index], attr->zp, attr->scale);
        float weight = expf(value - maxv);
        sum += weight;
        weighted += weight * (float)k;
    }
    return weighted / sum;
}
...
static int nms_and_cap(candidate_t *candidates, int count, float iou_thr, int max_det) {
    ...
    for (int i = 0; i < nms_count; i++) {
        ...
        if (kept < max_det) candidates[kept++] = selected;
        for (int j = i + 1; j < nms_count; j++) {
            if (!suppressed[j] && selected.cls == candidates[j].cls) {
                g_last_nms_pairs++;
                if (candidate_iou(&selected, &candidates[j]) <= iou_thr) continue;
                suppressed[j] = 1;
            }
        }
    }
...
static int collect_candidates(const rknn_output *outs,
    ...
    const int max_candidates = anchors * classes;
    candidate_t *candidates = ensure_candidates(max_candidates);
    ...
    const int count = paddleyolo_rknn::postprocess::SelectMultilabelSeedsInt8(
        scores, classes, anchors, {attrs[1].scale, attrs[1].zp}, conf_thr, score_sum_ptr,
        ...
        if (seed.anchor != cached_anchor) {
            if (use_dfl) {
                for (int side = 0; side < 4; ++side) {
                    cached_ltrb[side] = decode_dfl_side(
                        boxes, &attrs[0], seed.anchor, anchors, side, reg_max, transposed);
                }
            } else {
                for (int side = 0; side < 4; ++side) {
                    cached_ltrb[side] = dequant_i8_value(
                        boxes[(size_t)side * anchors + seed.anchor], attrs[0].zp,
                        attrs[0].scale);
                }
            }
    ...
    g_last_kept = count > 0 ? nms_and_cap(candidates, count, iou_thr, max_det) : 0;
    return g_last_kept;
}
...
static cv::Size make_mask_output_size(const candidate_t &candidate) {
    ...
    return cv::Size(static_cast<int>(std::max(0.0f, right - left)),
                    static_cast<int>(std::max(0.0f, bottom - top)));
}

static void compute_roi_mask_f32(const float *proto, int nm, int proto_h, int proto_w,
                                 const float *coeff, const cv::Rect &roi, cv::Mat &logits) {
    logits.create(roi.height, roi.width, CV_32F);
    logits.setTo(0.0f);
    const size_t plane_size = static_cast<size_t>(proto_h) * proto_w;
    for (int channel = 0; channel < nm; ++channel) {
        const float coefficient = coeff[channel];
        const float *plane = proto + static_cast<size_t>(channel) * plane_size;
        for (int y = 0; y < roi.height; ++y) {
            const float *src = plane + static_cast<size_t>(roi.y + y) * proto_w + roi.x;
            float *dst = logits.ptr<float>(y);
            int x = 0;
            const float32x4_t vcoefficient = vdupq_n_f32(coefficient);
            for (; x + 4 <= roi.width; x += 4) {
                float32x4_t accumulated = vld1q_f32(dst + x);
                accumulated = vmlaq_f32(accumulated, vcoefficient, vld1q_f32(src + x));
                vst1q_f32(dst + x, accumulated);
            }
            for (; x < roi.width; ++x) dst[x] += coefficient * src[x];
        }
    }
}

static void compute_roi_mask_i8(const int8_t *proto, int nm, int proto_h, int proto_w,
                                const float *coeff, float scale, int32_t zp,
    ...
    for (int channel = 0; channel < nm; ++channel) {
        const float coefficient = coeff[channel];
        const int8_t *plane = proto + static_cast<size_t>(channel) * plane_size;
        for (int y = 0; y < roi.height; ++y) {
            const int8_t *src = plane + static_cast<size_t>(roi.y + y) * proto_w + roi.x;
            float *dst = logits.ptr<float>(y);
            int x = 0;
            ...
            for (; x + 8 <= roi.width; x += 8) {
                const int16x8_t centered = vsubq_s16(vmovl_s8(vld1_s8(src + x)), vzp);
                const float32x4_t low = vmulq_f32(
                    vcvtq_f32_s32(vmovl_s16(vget_low_s16(centered))), vscale);
                const float32x4_t high = vmulq_f32(
                    vcvtq_f32_s32(vmovl_s16(vget_high_s16(centered))), vscale);
                float32x4_t accumulated_low = vld1q_f32(dst + x);
                float32x4_t accumulated_high = vld1q_f32(dst + x + 4);
                accumulated_low = vmlaq_f32(accumulated_low, vcoefficient, low);
                accumulated_high = vmlaq_f32(accumulated_high, vcoefficient, high);
                vst1q_f32(dst + x, accumulated_low);
                vst1q_f32(dst + x + 4, accumulated_high);
            }
            for (; x < roi.width; ++x) {
                const float value =
                    (static_cast<float>(static_cast<int32_t>(src[x]) - zp)) * scale;
                dst[x] += coefficient * value;
            }
        }
    }
}
...
static void run_segmentation_tail(const rknn_output *outs,
                                  const rknn_tensor_attr *attrs) {
    const int kept = g_last_kept;
    ...
    const int8_t *proto_i8 = static_cast<const int8_t *>(outs[3].buf);

    float *coeff = ensure(&g_mb, &g_lm, kept * nm);
    const int8_t *coeff_q = static_cast<const int8_t *>(outs[2].buf);
    for (int i = 0; i < kept; ++i) {
        const int anchor = g_candidates[i].anchor;
        for (int channel = 0; channel < nm; ++channel) {
            coeff[static_cast<size_t>(i) * nm + channel] = dequant_i8_value(
                coeff_q[static_cast<size_t>(channel) * anchors + anchor],
                attrs[2].zp, attrs[2].scale);
        }
        g_last_proto_roi_area += make_proto_roi(g_candidates[i], proto_h, proto_w).area();
    }

    const bool use_int8_roi = g_last_proto_roi_area <= kAutoRoiTiledInt8MaxArea;
    ...
    if (!use_int8_roi) {
        ...
        neon_dequant_i8_f32(proto_i8, proto_f32, static_cast<size_t>(nm) * proto_pixels,
    ...
    for (int i = 0; i < kept; ++i) {
        const cv::Rect roi = make_proto_roi(g_candidates[i], proto_h, proto_w);
        const cv::Size output_size = make_mask_output_size(g_candidates[i]);
        if (roi.empty() || output_size.width <= 0 || output_size.height <= 0) continue;

        const float *coeff_row = coeff + static_cast<size_t>(i) * nm;
        if (use_int8_roi) {
            compute_roi_mask_i8(proto_i8, nm, proto_h, proto_w, coeff_row,
                                attrs[3].scale, attrs[3].zp, roi, g_roi_logits);
        } else {
            compute_roi_mask_f32(proto_f32, nm, proto_h, proto_w, coeff_row, roi,
                                 g_roi_logits);
        }
        neon_sigmoid_inplace(g_roi_logits.ptr<float>(), g_roi_logits.total());
        cv::resize(g_roi_logits, g_mask_resized, output_size, 0.0, 0.0, cv::INTER_LINEAR);
        g_mask_resized.convertTo(g_mask_binary, CV_8U, 255.0);
        cv::threshold(g_mask_binary, g_mask_binary, 127, 255, cv::THRESH_BINARY);
...
static double postproc_detect(const rknn_output *outs,
                              ...
                              int max_det) {
    double start = now_ms();
    collect_candidates(outs, attrs, use_dfl, use_score_sum, conf_thr, iou_thr, max_det);
    if (segmentation) run_segmentation_tail(outs, attrs);
...
static double postproc_predist(const rknn_output *outs,
    ...
    return postproc_detect(outs, attrs, false, false, false, conf_thr, iou_thr, max_det);
}
...
static double postproc_predfl(const rknn_output *outs,
                              ...
                              int max_det) {
    if (n_out != 2) return 0.0;
    return postproc_detect(outs, attrs, true, false, false, conf_thr, iou_thr, max_det);
}
...
static double postproc_seg_predist(const rknn_output *outs,
    ...
    return postproc_detect(outs, attrs, false, true, true, conf_thr, iou_thr, max_det);
}
...
static double postproc_seg_predfl(const rknn_output *outs,
                                  ...
                                  int max_det) {
    if (n_out != 5) return 0.0;
    return postproc_detect(outs, attrs, true, true, true, conf_thr, iou_thr, max_det);
}
...
static int validate_quantized_output(const rknn_tensor_attr *attr, uint32_t index) {
    if (attr->type != RKNN_TENSOR_INT8 ||
        attr->qnt_type != RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC || attr->scale <= 0.f) {
        fprintf(stderr,
                "output[%u] must be affine INT8 with positive scale, got type=%d qnt=%d scale=%g\n",
...
static int validate_output_contract(pp_mode_t mode,
                                    uint32_t n_output,
                                    const rknn_tensor_attr *attrs) {
    if (mode == PP_NONE) return 0;
    uint32_t expected = (mode == PP_SEG_PREDIST || mode == PP_SEG_PREDFL) ? 5u : 2u;
    ...
    for (uint32_t i = 0; i < n_output; i++) {
        if (validate_quantized_output(&attrs[i], i) < 0) return -1;
    }
    ...
    if (expected == 5u) {
        ...
        if (attrs[3].n_dims != 4 || attrs[3].dims[0] != 1 ||
            attrs[3].dims[1] != (uint32_t)nm || attrs[3].dims[2] < 1 || attrs[3].dims[3] < 1) {
...
static double run_postproc(pp_mode_t pp,
    ...
    switch (pp) {
        case PP_PREDIST:
            return postproc_predist(outs, out_attrs, n_output, conf_thr, iou_thr, max_det);
        case PP_PREDFL:
            return postproc_predfl(outs, out_attrs, n_output, conf_thr, iou_thr, max_det);
        case PP_SEG_PREDIST:
            return postproc_seg_predist(outs, out_attrs, n_output, conf_thr, iou_thr, max_det);
        case PP_SEG_PREDFL:
            return postproc_seg_predfl(outs, out_attrs, n_output, conf_thr, iou_thr, max_det);
        default:
            return 0.0;
    }
}
...
static int setup_context(rknn_context *ctx,
    ...
    rknn_set_core_mask(*ctx, static_cast<rknn_core_mask>(core_mask));
    ...
    if (validate_output_contract(pp, io->n_output, out_attrs) < 0) return -1;
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv, "m:w:r:c:p:t:j:s:h", opts, NULL)) != -1) {
        switch (opt) {
            ...
            case 1003: iou_thr = (float)atof(optarg); break;
            case 1004: max_det = atoi(optarg); break;
            case 1005: score_sum_str = optarg; break;
            case 1006: input_path = optarg; break;
    ...
    if (warmup < 0 || runs <= 0 || !std::isfinite(conf_thr) || conf_thr < 0.f ||
        conf_thr > 1.f || !std::isfinite(iou_thr) || iou_thr < 0.f || iou_thr > 1.f ||
        max_det <= 0 || fps_workers < 0 || !std::isfinite(fps_seconds) || fps_seconds <= 0.0) {
    ...
    rknn_set_core_mask(ctx, static_cast<rknn_core_mask>(parse_core(core_str)));
    ...
    for (int i = 0; i < runs; i++) {
        ...
        if (ret < 0) {
            ...
            free(npu_times); free(io_wall_times); free(e2e_times);
            free_postproc_buffers(); free(dummy); rknn_destroy(ctx);
            return 1;
        }
        ret = rknn_run(ctx, NULL);
        if (ret < 0) {
    ...
    for (int i = 0; i < runs - 1; i++)
        for (int j = i + 1; j < runs; j++)
            if (io_wall_times[j] < io_wall_times[i]) {
                double t = io_wall_times[i]; io_wall_times[i] = io_wall_times[j]; io_wall_times[j] = t;
            }
    for (int i = 0; i < runs - 1; i++)
        for (int j = i + 1; j < runs; j++)
    ...
    printf("npu_pure_ms: best=%.3f  P50=%.3f  P90=%.3f  avg=%.3f  (RKNN_QUERY_PERF_RUN)\n",
           npu_best, npu_p50, npu_p90, npu_avg_ms);
    printf("io_wall_ms:  best=%.3f  P50=%.3f  P90=%.3f  avg=%.3f\n",
           io_wall_best, io_wall_p50, io_wall_p90, io_wall_avg_ms);
    printf("postproc_ms: %.3f\n", pp_avg_ms);
    if (pp != PP_NONE) {
    ...
    if (json_out) {
        ...
        if (jp) {
            fprintf(jp,
                ...
                "\"npu_pure_best_ms\":%.4f,\"npu_pure_p50_ms\":%.4f,\"npu_pure_p90_ms\":%.4f,\"npu_pure_avg_ms\":%.4f,"
                "\"io_wall_best_ms\":%.4f,\"io_wall_p50_ms\":%.4f,\"io_wall_p90_ms\":%.4f,\"io_wall_avg_ms\":%.4f,"
                "\"postproc_ms\":%.4f,\"postproc_candidates\":%d,\"postproc_kept\":%d,\"nms_pairs\":%ld,"
                "\"score_sum_applied\":%d,\"score_sum_scanned\":%d,"
                "\"class_anchors_scanned\":%d,\"class_values_scanned\":%ld,"
                "\"mask_mode\":\"%s\",\"mask_proto_roi_area\":%ld,"
                "\"mask_resized_pixels\":%ld,\"mask_active_pixels\":%ld,\"mask_hash\":\"%016llx\","
                ...
                npu_best, npu_p50, npu_p90, npu_avg_ms,
                io_wall_best, io_wall_p50, io_wall_p90, io_wall_avg_ms,
                pp_avg_ms, g_last_candidates, g_last_kept, g_last_nms_pairs,
                g_last_score_sum_applied, g_last_sum_scanned, g_last_class_anchors,
                g_last_class_values,
                g_last_mask_mode, g_last_proto_roi_area, g_last_mask_pixels, g_last_mask_active,
                (unsigned long long)g_last_mask_hash,
