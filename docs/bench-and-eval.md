...
- 四/五输出分割强制使用 full-IO zero-copy：输入和全部输出通过
...
- YOLO26 按 `cls → box → mask_coeff + proto`，YOLOv8 按
  `score_sum → cls → DFL → mask_coeff + proto` 分阶段同步；
...
- YOLO26 执行 stable 两阶段 exact TopK，允许同 anchor 多类别且不执行 NMS；
  YOLOv8 每个 anchor 保留最高类别并执行 class-aware NMS
...
- 输出延迟统计（best / P50 / P90 / avg）：
  ...
  - `e2e_ms`：端到端墙钟耗时
- full-IO 路线额外报告 `output_sync_ms`、`native_sync_bytes`、`ready_mask`
...
bench_rknn_perf
  ...
  --score-sum on|off               # 仅 YOLOv8-Seg 五输出预筛 A/B
...
# YOLOv8-Seg 五输出完整后处理；输入为 HWC RGB uint8 原始帧
./bench_rknn_perf \
...
mask 类别白名单在候选选择后、同步 `mask_coeff + proto` 前应用。若本帧没有
白名单类别，bench 以 `fetch_outcome=no_mask_classes` 提前结束，避免两块
mask 输出的 DMA 同步和解码。做跨实现 A/B 时必须显式对齐 `--iou-thr`、
`--mask-class-ids` 与 `--mask-output-size`；模型族、候选语义和最终 mask 尺度不同的
结果不能直接比较 E2E。

当前 YOLOv8-Seg 640 五输出模型的原生输出总计 `1,894,400 B`。按需同步的理论口径：
...
- 10 类 YOLOv8-Seg 640 五输出模型的真实图像 A/B 中，`score_sum=on` 实际读取
