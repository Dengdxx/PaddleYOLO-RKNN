...
**用途：** RKNN INT8 量化时跳过 NMS 与 DFL 子图，保留 raw head 输出；decode/NMS 由 CPU + NEON 后处理补回。

分割头的四个 `dual_raw` 张量只作为导出中间图；进入量化和 RKNN 编译前，
`seg_predist` 与 `seg_predfl` 都会追加 `score_sum` 并固定为五输出。
批量导出复用缓存前会验证该契约；旧四输出 ONNX 会自动失效并重建，
由其生成的 RKNN 也会随五输出源模型更新而重新编译。
