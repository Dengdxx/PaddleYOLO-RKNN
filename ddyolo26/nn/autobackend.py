            ...
            for k, v in metadata.items():
                if k in {"stride", "batch", "channels"}:
                    ...
                    try:
                        ...
                        # 兼容旧导出器写入的 ImageShape(height=H, width=W)
                        # 表示；统一转换为后端使用的 [H, W] 列表。
                        if k == "imgsz":
                            match = re.fullmatch(r"ImageShape\(height=(\d+),\s*width=(\d+)\)", v)
                            if match:
                                metadata[k] = [int(match.group(1)), int(match.group(2))]
                            else:
                                raise
                        else:
                            raise
